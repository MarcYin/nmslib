...
struct IndexWrapper {
  ...
  py::object knnQueryBatch(py::object input, size_t k, int num_threads, bool index_only) {
    if (!index) {
      throw std::invalid_argument("Must call createIndex or loadIndex before this method");
    }
    ...
    for (auto & result : results) {
      ...
      if(index_only){
        ret.append(convertResultIndexOnly(result.get()));
        }
      else {
  ...
  py::object convertResult(KNNQueue<dist_t> * res) {
    ...
    return make_tuple(ids, distances);
  }

  py::object convertResultIndexOnly(KNNQueue<dist_t> * res) {
    // Create numpy arrays for the output
    size_t size = res->Size();
    py::array_t<int> ids(size);
    //py::array_t<dist_t> distances(size);
...
void exportIndex(py::module * m) {
  ...
  py::class_<IndexWrapper<dist_t>>(*m, index_name.c_str())
    ...
    .def("knnQueryBatch", &IndexWrapper<dist_t>::knnQueryBatch,
      py::arg("queries"), py::arg("k") = 10, py::arg("num_threads") = 0, py::arg("index_only") = false,
      ...
      "    Only return index or not default to beFalse\n"
      "\n"
      "Returns\n"
      "----------\n"
      "list:\n"
      "   A list of tuples of (ids, distances) or (ids if index_only=True)\n ")
